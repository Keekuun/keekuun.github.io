...
- 全量中断后：下载 Actions 产物 `rag-state-cache`，放到 `tools/rag/`，次日设 `RAG_RESUME_UPSERT_FROM`（见 `.upsert-progress.json` 里的 offset）再跑；或仓库 Variable `RAG_RESUME_UPSERT_FROM=4800`。
